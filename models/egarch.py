...
class EGARCHModel:
    def __init__(self, p: int = 1, o: int = 1, q: int = 1, scale: float = RETURN_SCALE,
                 n_bars_per_year: int = N_BARS_PER_YEAR):
        self.p = p
        self.o = o
        self.q = q
        self.scale = scale
        self.n_bars_per_year = n_bars_per_year
    ...
    def forecast(self, horizon: int) -> np.ndarray:
        ...
        annualized = fcast_vols * np.sqrt(self.n_bars_per_year)
        return annualized
    ...
    def save(self, path: str) -> None:
        data = {
            ...
            "q": self.q,
            "scale": self.scale,
            "n_bars_per_year": self.n_bars_per_year,
            "result": self._result,
    ...
    @classmethod
    def load(cls, path: str) -> EGARCHModel:
        with open(path, "rb") as f:
            data = pickle.load(f)
        model = cls(p=data["p"], o=data["o"], q=data["q"], scale=data["scale"],
                    n_bars_per_year=data.get("n_bars_per_year", 252 * 26))
